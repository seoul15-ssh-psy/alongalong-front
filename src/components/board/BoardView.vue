<template>
  ...
  <table >
		...
		<img :src="'http://localhost:9999/vue/file/'+article.articleno" id="fileImg" style="display:none">
		<a v-if="article.originalfile" :href="'http://localhost:9999/vue/file/download/'+article.savefolder+'/'+article.originalfile+'/'+article.savefile"><button >다운로드</button></a>
	</table>
    <div id="buttons">
  ...
  created() {
    ...
    getArticle(
      ...
      ({ data }) => {
		  ...
		  if (this.article.originalfile.split('.')[1] != undefined && imgPrefix.includes(this.article.originalfile.split('.')[1])) {
