<template>
  <div class="q-mx-xs">
    <!--큰 메뉴-->
    <div
      class="gt-xs full-width row justify-between items-center fixed-top q-pl-lg"
      id="headNavBigMenu"
      :style="[
        { zIndex: '5' },
        { backgroundColor: headerNavColor },
        { boxShadow: headerNavShadow },
        { paddingLeft: headerNavLogoPad + 'px' }
      ]"
    >
      <q-img
        id="logoImg"
        src="../../../public/icons/mainIcon1.png"
      ...
      <div class="row">
        <div class="col-12 q-ml-md">
          <a
            class="headerNavMenu clickable"
            @click="routerPush('map')"
            :style="[
              { fontSize: headerNavMenuSize + 'px' },
              { color: linkSelected[1] }
            ]"
            >지역별 여행지</a
          >
          <a
            class="headerNavMenu clickable"
            @click="routerPush('hotPlace')"
            :style="[
              { paddingLeft: headerNavMenuPad + 'px' },
              { fontSize: headerNavMenuSize + 'px' },
              { color: linkSelected[2] }
            ]"
            >핫플 게시판</a
          >
          <a
            class="headerNavMenu clickable"
            @click="routerPush('myPlan')"
            :style="[
              { paddingLeft: headerNavMenuPad + 'px' },
              { fontSize: headerNavMenuSize + 'px' },
              { color: linkSelected[3] }
            ]"
            >나의 여행계획</a
          >
          <a
            class="headerNavMenu clickable"
            @click="routerPush('boardList')"
            :style="[
              { paddingLeft: headerNavMenuPad + 'px' },
              { fontSize: headerNavMenuSize + 'px' },
              { color: linkSelected[4] }
            ]"
            >자유게시판</a
          >
        </div>
      ...
      <q-avatar class="q-mr-md" size="35px">
        ...
        <q-menu touch-position>
          <q-list dense style="min-width: 100px">
            ...
            <q-item clickable v-close-popup v-if="!getIsLogin">
              <q-item-section @click="showRegisterModal"
                >회원가입</q-item-section
              >
            </q-item>
            <q-item clickable v-close-popup v-if="getIsLogin">
            ...
            <q-item clickable v-close-popup v-if="getIsLogin">
              <q-item-section @click="routerPush('myPage')"
                >마이페이지</q-item-section
              >
            </q-item>
          </q-list>
        </q-menu>
    ...
    <!--작은 메뉴-->
    <div
      class="lt-sm full-width q-pb-sm fixed-top"
      :style="[
        { zIndex: '5' },
    ...
    >
      <div class="row justify-center items-center">
        <img
          id="logoImg2"
          src="../../../public/icons/mainIcon2.png"
          @click="routerPush('main')"
          class="clickable"
        />
      ...
      <div class="row full-width justify-around">
        <a
          class="headerNavMenu2 clickable"
          @click="routerPush('map')"
          :style="{ color: linkSelected[1] }"
          >지역별 여행지</a
        >
        <a
          class="headerNavMenu2 clickable"
          @click="routerPush('hotPlace')"
          :style="{ color: linkSelected[2] }"
          >핫플 게시판</a
        >
        <a
          class="headerNavMenu2 clickable"
          @click="routerPush('myPlan')"
          :style="{ color: linkSelected[3] }"
          >나의 여행계획</a
        >
        <a
          class="headerNavMenu2 clickable"
          @click="routerPush('boardList')"
          :style="{ color: linkSelected[4] }"
          >자유게시판</a
        >
      </div>
    </div>
